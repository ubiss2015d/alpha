...
function readVenues()
{
	...
	$.ajax({
		...
		error: function(jqXHR, exception) {
			if (jqXHR.status === 0) {
				console.log('handleData: Not connect.\n Verify Network.');
			} else if (jqXHR.status == 404) {
				console.log('handleData:Requested page not found. [404]');
			} else if (jqXHR.status == 500) {
				console.log('handleData:Internal Server Error [500].');
			} else if (exception === 'parsererror') {
				console.log('handleData:Requested JSON parse failed.');
			} else if (exception === 'timeout') {
				console.log('handleData:Time out error.');
			} else if (exception === 'abort') {
				console.log('handleData:Ajax request aborted.');
			} else {
				console.log('handleData:Uncaught Error.\n' + jqXHR.responseText);
			}
...
function readStats()
{
	...
	$.ajax({
		//url: 'getStatData.php',
        url: "http://localhost:8082/data/pno_tilasto_2015_evesav.csv",
		type: 'GET',
		cache: false,
        dataType: "text",
		success: function(response) {
			...
			//initializeMap();
			//427983.5685,7210660.3856;
			var point = [65.015763,25.4713242];
			console.log('transformed :'+transformWGS84toETRS(point));

			var closestDistrictIndex=findClosestDistrict([423630.0795999998, 7205290.2518999991]);

			console.log('closest district :'+closestDistrictIndex);
			
			var boundaryCoordinates=getDistrictBoundaries(closestDistrictIndex);	
			console.log('boundary coords '+boundaryCoordinates);
			...
			*/
			...
			//var download_url=downloadFile('https://drive.google.com/file/d/0B6-Yt8cS7VIUVFNrMlIyLWVqSU0/view?usp=sharing', 'j');
...
//returns coordinates of district boundaries in WGS84
function getDistrictBoundaries(index) {

		var boundaryArray;


		for (var key in coordArrayAng)
		{
			if(index==key)
			{
					boundaryArray=coordArrayAng[key];
			}
...
function downloadFile(fileUrl, accessToken) {
...
function pointInDistrict(){
	...
	var districtName='';
		...
		for (var key in coordArrayRect)
		{
		    	...
		    	console.log('Mindist from'+statData[key][2]+' is '+calcMinDist(point,coordArrayRect[key])[0]);
		    	//helpoer
		    	if(key==5){
